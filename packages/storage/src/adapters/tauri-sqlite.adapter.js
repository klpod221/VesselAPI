...
export class TauriSQLiteAdapter {
    ...
    async getAllCollections() {
        ...
        const result = await this.db.select('SELECT * FROM collections ORDER BY updated_at DESC');
        return result.map((row) => this.rowToCollection(row));
    }
    async getCollection(id) {
        if (!this.db)
            throw new Error('Database not initialized');
        const result = await this.db.select('SELECT * FROM collections WHERE id = ?', [id]);
        if (result.length === 0)
            return null;
    ...
    async saveCollection(collection) {
        if (!this.db)
            throw new Error('Database not initialized');
        await this.db.execute(`INSERT OR REPLACE INTO collections 
    ...
    async getKV(key) {
        if (!this.db)
            throw new Error('Database not initialized');
        const result = await this.db.select('SELECT value FROM key_value WHERE key = $1', [key]);
        if (result.length === 0)
            return null;
    ...
    async setKV(key, value) {
        if (!this.db)
            throw new Error('Database not initialized');
        // UPSERT equivalent
        await this.db.execute('INSERT OR REPLACE INTO key_value (key, value, updated_at) VALUES ($1, $2, $3)', [key, value, Date.now()]);
    }
    async deleteKV(key) {
        if (!this.db)
            throw new Error('Database not initialized');
    ...
    rowToCollection(row) {
        return {
            ...
            storageMode: row.storage_mode || 'local',
...
//# sourceMappingURL=tauri-sqlite.adapter.js.map